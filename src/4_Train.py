...
from tensorflow.keras.layers import LSTM, Dense, Dropout, Masking, Input, Bidirectional, BatchNormalization
...
class AutismDetectionTrainer:
    ...
    def load_data(self):
        ...
        try:
            ...
            # NaN temizliği
            for name, data in [("X_train", self.X_train), ("X_val", self.X_val), ("X_test", self.X_test)]:
                if np.isnan(data).any():
                    print(f"NaN tespit edildi ({name}), temizleniyor...")
                    setattr(self, name, np.nan_to_num(data))

            # 🎯 Hafif gürültü ekleme (data augmentation)
            noise = np.random.normal(0, 0.01, self.X_train.shape)
    ...
    def create_model(self, input_shape):
        """
        Daha az parametreli, L2 regularization ve batch normalization içeren LSTM modeli
        """
        model = Sequential([
            Input(shape=input_shape),
            Masking(mask_value=0.),
            Bidirectional(LSTM(64, return_sequences=True, dropout=0.4, recurrent_dropout=0.3,
                               kernel_regularizer=l2(0.001))),
            BatchNormalization(),
            Bidirectional(LSTM(32, dropout=0.4, recurrent_dropout=0.3, kernel_regularizer=l2(0.001))),
            BatchNormalization(),
